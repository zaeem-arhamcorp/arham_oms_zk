...
import 'package:arham_corporation/views/payable_confirm_payment_settlement_page.dart';
import 'package:arham_corporation/views/payable_report_payment_page.dart';
...
class PayablePaymentSettlementPage extends StatefulWidget {
  @override
  _PayablePaymentSettlementPageState createState() =>
      _PayablePaymentSettlementPageState();
}

class _PayablePaymentSettlementPageState
    extends State<PayablePaymentSettlementPage> {
    ...
    setState(() {
      if (selected) {
        ...
        billwise = selectedPurchases.map((data) {
          double vouchAmt;
          if (data.vouchAmt is String) {
            vouchAmt = double.tryParse(data.vouchAmt) ?? 0.0;
          } else if (data.vouchAmt is double) {
            vouchAmt = data.vouchAmt;
          } else {
            vouchAmt =
                0.0; // Default value if vouchAmt is neither String nor double
          }
          double osAmt;
          if (data.osAmt is String) {
            osAmt = double.tryParse(data.osAmt) ?? 0.0;
          } else if (data.osAmt is double) {
            osAmt = data.osAmt;
          } else {
            osAmt = 0.0; // Default value if osAmt is neither String nor double
          }
          return {
            ...
            "billAmt": vouchAmt * -1, // Multiply billAmt by -1
            "drCrNote": "",
            "kasar": "",
            "paid": osAmt * -1, // Use the parsed osAmt
          };
        }).toList();
      } else {
  ...
  void togglePurchaseSelection11(
      bool selected, DatumPartyWiseOutstandingSale purchase) {
    setState(() {
  ...
  void togglePurchaseSelection111(
      bool selected, DatumPartyWiseOutstandingSale purchase) {
    setState(() {
      final amountText = autoAmountController.text;

      double purchaseAmount =
          double.tryParse(purchase.osAmt?.toString() ?? '0') ?? 0.0;

      if (selected) {
        // Block if trying to add and remainingAmount is 0
        if (amountText.isNotEmpty && remainingAmount <= 0) {
          AppSnackBar.showGetXCustomSnackBar(
            message: "Remaining amount is 0, cannot select more bill.",
          );
          return;
        ...
        // Adjust remainingAmount
        if (purchase.vouchType == 'D') {
          remainingAmount += purchaseAmount.abs();
        } else if (purchase.vouchType == 'C') {
          remainingAmount -= purchaseAmount.abs();
        ...
        // Reverse adjust remainingAmount
        if (purchase.vouchType == 'D') {
          remainingAmount -= purchaseAmount.abs();
          if (remainingAmount < 0) remainingAmount = 0;
        } else if (purchase.vouchType == 'C') {
      ...
      billwise = selectedPurchases.map((data) {
        return {
          ...
          "billNo": data.partyBl,
          "billDate": data.vouchDt,
          "billAmt": double.parse(
            double.parse(data.vouchAmt!.toString()) < 0
                ? (-1 * double.parse(data.vouchAmt!.toString()))
                    .toStringAsFixed(2)
                : double.parse(data.vouchAmt!.toString()).toStringAsFixed(2),
          ),
          ...
          "kasar": "",
          "paid": double.parse(
            double.parse(data.osAmt!.toString()) < 0
                ? (-1 * double.parse(data.osAmt!.toString())).toStringAsFixed(2)
                : double.parse(data.osAmt!.toString()).toStringAsFixed(2),
          ),
        };
      }).toList();
      ...
      distributeFIFOAmount(); // Call after updating billwise and remainingAmount
  ...
  void togglePurchaseSelection(
    ...
    setState(() {
      ...
      // Parse the outstanding amount of the current purchase, ensuring it's always positive for calculation
      double purchaseAmount =
          double.tryParse(purchase.osAmt?.toString() ?? '0') ?? 0.0;
      // Always use the absolute value for calculations of the individual purchase amount
      final absPurchaseAmount = purchaseAmount.abs();

      if (selected) {
        // If adding a purchase:
        // Block selection if an amount is entered and remaining amount is 0 or less
        if (amountText.isNotEmpty && remainingAmount <= 0) {
          AppSnackBar.showGetXCustomSnackBar(
            message: "Remaining amount is 0, cannot select more bills.",
          );
          return;
        }

        // Add the purchase to the selected list
        selectedPurchases.add(purchase);

        // Adjust remainingAmount based on vouchType:
        // 'D' (Debit) type: Increases the remaining amount to be settled (outstanding)
        if (purchase.vouchType == 'D') {
          remainingAmount += absPurchaseAmount;
        }
        // 'C' (Credit) type: Decreases the remaining amount to be settled (reduces outstanding)
        else if (purchase.vouchType == 'C') {
          remainingAmount -= absPurchaseAmount;
          // Ensure remainingAmount does not go below zero when decreasing
          if (remainingAmount < 0) remainingAmount = 0;
        }
      } else {
        // If unselecting a purchase:
        // Remove the purchase from the selected list
        selectedPurchases.remove(purchase);

        // Reverse adjust remainingAmount based on vouchType:
        // 'D' (Debit) type: Decreases the remaining amount (reverses addition)
        if (purchase.vouchType == 'D') {
          remainingAmount -= absPurchaseAmount;
          // Ensure remainingAmount does not go below zero when decreasing
          if (remainingAmount < 0) remainingAmount = 0;
        }
        // 'C' (Credit) type: Increases the remaining amount (reverses subtraction)
        else if (purchase.vouchType == 'C') {
          remainingAmount += absPurchaseAmount;
        }
      }

      // Build the billwise list from the currently selected purchases
      billwise = selectedPurchases.map((data) {
        // Parse outstanding amount from data, always taking the absolute value for base calculation
        final currentOsAmt = double.parse(data.osAmt?.toString() ?? '0').abs();
        // Parse voucher amount from data, always taking the absolute value for base calculation
        final currentVouchAmt =
            double.parse(data.vouchAmt?.toString() ?? '0').abs();
        ...
        return {
          ...
          "billNo": data.partyBl,
          "billDate": data.vouchDt,
          // billAmt: Always positive, based on the absolute voucher amount
          "billAmt": currentVouchAmt,
          "drCrNote": "",
          "kasar": "",
          // paid: Settlement amount sign logic based on vouchType
          // For 'D' (Debit) type, paid should be negative
          // For 'C' (Credit) type, paid should be positive
          "paid": data.vouchType == 'D' ? -currentOsAmt : currentOsAmt,
        };
      }).toList();

      // Sort the billwise list: first by vouchType, then by billDate
      billwise.sort((a, b) {
        final vouchTypeA = a["vouchType"].toString();
        final vouchTypeB = b["vouchType"].toString();
        //final vouchTypeComparison = vouchTypeA.compareTo(vouchTypeB);
        final vouchTypeComparison = vouchTypeB.compareTo(vouchTypeA);
        if (vouchTypeComparison != 0) return vouchTypeComparison;
      ...
      // Call distributeFIFOAmount after all relevant state variables (billwise, remainingAmount) are updated
      distributeFIFOAmount();
  ...
  void finalizePaymentQuickSave() {
    if (billwise.isEmpty) {
      ...
      setState(() {
        Services()
            .finalPayableReceiptPaymentQuickSave(
          context,
          ...
          if (val == true) {
            ...
            setState(() {
              ...
              //   context,
              //   MaterialPageRoute(
              //     builder: (context) => PayablePaymentSettlementPage(),
              //   ),
              // );

              Get.to(
                PayablePaymentSettlementPage(),
                arguments: {
  ...
  void finalizePaymentManuallySave() {
    if (selectedBankName.isEmpty) {
      ...
      setState(() {
        Services()
            .finalPayableReceiptPaymentManuallySave(
          context,
          Helper.toApi(toDateController.text),
          formattedTime,
          selectedBankId,
  ...
  void showPartySelectionMenu1(BuildContext context) {
    ...
    pp.getpartynameForPaymentWithoutFilter(context); // API call to fetch party data
  ...
  void showPartySelectionMenu(BuildContext context) {
    final PartyProvider pp = Provider.of<PartyProvider>(context, listen: false);
    pp.getpartynameForPayment(context); // API call to fetch party data
  ...
  getDate() {
    ...
    Services()
        .getPartyWiseOutStandingPayableReport(
        ...
        if (value != null) {
          ...
          for (var item in data) {
            ...
            if (dueDate.isBefore(currentDate)) {
              // Add the negated OS_AMT to the total sum
              totalOSAmount += -(double.tryParse(item.osAmt.toString()) ?? 0.0);
            }
          }
  ...
  void initState() {
    ...
    paymentDeleteRight = Get.arguments['DeleteRight'];
    paymentReadRight = Get.arguments['ReadRight'];
                        ...
                        children: [
                          Visibility(
                            ...
                            child: Expanded(
                              child: ElevatedButton(
                                onPressed: () {
                                  // Get.to(() => PayableReportPaymentPage(),
                                  //     arguments: {
                                  //       "DeleteRight": paymentDeleteRight,
                                  //       'ReadRight': paymentReadRight,
                                  //     });
                            ...
                            child: Expanded(
                              child: ElevatedButton(
                                onPressed: () {
                                  ...
                                  //   context,
                                  //   MaterialPageRoute(
                                  //     builder: (context) => PayableConfirmPaymentSettlementPage(),
                                  //   ),
                                  // );

                                  Get.to(
                                      () =>
                                          PayableConfirmPaymentSettlementPage(),
                                      arguments: {
                                        "DeleteRight": paymentDeleteRight,
                                      });
                                },
                                                                ...
                                                                Expanded(
                                                                  child: Column(
                                                                    ...
                                                                    children: [
                                                                      ...
                                                                      Text(
                                                                        "${getFormattedAmount(data[index].vouchAmt, data[index].vouchType)}",
                                                                        style:
                                                                            TextStyle(
                                                                          fontSize:
                                                                              12,
                                                                      ...
                                                                      // )
                                                                      /* Text(
                                                                        "${Helper.parseNumericeValue(double.parse(data[index].vouchAmt).isNegative
                                                                        ?(-double.parse(data[index].vouchAmt)).toString():data[index].vouchAmt
                                                                        )??""}",
                                                                        style: TextStyle(
                                                                            fontSize: 12,
                                                                            color: Colors.grey),
                                                                ...
                                                                Expanded(
                                                                  child: Column(
                                                                    ...
                                                                    children: [
                                                                      ...
                                                                      Text(
                                                                        "${getFormattedAmount(data[index].paidAmt, data[index].vouchType)}",
                                                                        style:
                                                                            TextStyle(
                                                                          fontSize:
                                                                              12,
                                                                ...
                                                                Expanded(
                                                                  child: Column(
                                                                    ...
                                                                    children: [
                                                                      ...
                                                                      Text(
                                                                        "${getFormattedAmount(data[index].osAmt, data[index].vouchType)}",
                                                                        style:
                                                                            TextStyle(
                                                                          fontSize:
                                                                              12,
  ...
  // Function to handle numeric formatting based on vouchType
  String getFormattedAmount(dynamic value, String vouchType) {
    double amount = double.tryParse(value.toString()) ?? 0.0;

    if (vouchType == 'C') {
      // Credit: Keep positive as is, convert negative to positive
      amount = amount < 0 ? amount * -1 : amount;
    } else if (vouchType == 'D') {
      // Debit: Keep negative as is, convert positive to negative
      amount = amount > 0 ? amount * -1 : amount;
    }

    return Helper.parseNumericValue(amount.toStringAsFixed(2));
  }
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      //     final purchase = selectedPurchases[index];
                      //     return ListTile(
                      //       title: Text("Vouch No : ${purchase.vouchNo}"),
                      //       subtitle: Text(
                      //         'Due Amount : ₹ ${formatAmount(-(double.tryParse(purchase.osAmt.toString()) ?? 0.0))}',
                      //       ),
                      //     );
                        ...
                        children: [
                          ...
                          InputDecorator(
                            decoration: InputDecoration(
                              ...
                              labelText: 'Search Party (Name, Phone Number, City, Area)',
                              border: OutlineInputBorder(),
                            ),
                            child: TextButton(
