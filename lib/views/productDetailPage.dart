          ...
          child: Column(
            ...
            children: [
              ...
              _buildInfoCard("Pricing Details", [
                ...
                if (_canLabelSettings(context.read<ProfileProvider>()))
                  _buildConditionalRow(
                    Icons.local_offer, "Net Rate", widget.data.nrate,
                    // Helper.parseNumericValue(widget.data.nrate),
                  ),
                if (ub.role == AppConfig.masteruser) ...[
                  _buildConditionalRow(
                    Icons.price_change, "Purch Rate", widget.data.prate,
                    // Helper.parseNumericValue(widget.data.prate),
                  ),
                  _buildConditionalRow(
                    Icons.percent, "Purch Disc", widget.data.pdisc,
                    // Helper.parseNumericValue(widget.data.pDisc),
                  ),
                  _buildConditionalRow(
                      Icons.rate_review,
                      "Net Landing",
                      (widget.data.tLAND?.toString().isNotEmpty ?? false)
                          ? widget.data.tLAND
              ...
              _buildInfoCard("Stock and Other Details", [
                ...
                if (widget.data.exDt!.isNotEmpty)
                  _buildDetailRow(
                    Icons.date_range,
                    "Expiry Date",
                    Helper.toUi(
                      widget.data.exDt.toString(),
                    ),
                  ),
                _buildDetailRow(
                  Icons.view_list,
