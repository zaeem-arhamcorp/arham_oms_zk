...
class ItemWisePartyWisePurchaseReportScreen extends StatefulWidget {
  const ItemWisePartyWisePurchaseReportScreen({Key? key}) : super(key: key);

  @override
  State<ItemWisePartyWisePurchaseReportScreen> createState() =>
      _ItemWisePartyWisePurchaseReportScreenState();
}

class _ItemWisePartyWisePurchaseReportScreenState
    extends State<ItemWisePartyWisePurchaseReportScreen> {
...
// Call this method to search for data when the search input changes.
  ...
  getDate() {
    ...
    Services()
        .getPartyWiseItemPurchaseReport(
            context,
...
///////////////////////////////////////////////////////////////
          ...
          return Consumer<PartyProvider>(
            builder: (context, party, child) {
              return SafeArea(
                top: false,
                child:
                    StatefulBuilder(builder: (context, StateSetter setStatee) {
                  final Global global = context.watch<Global>();
                  return Padding(
                    padding: MediaQuery.of(context).viewInsets,
                    child: Container(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CustomAppBar(
        title: "Party Wise Item Wise Purchase Report",
        actions: [
          if (printRight)
                  ...
                  PopupMenuItem(
                    ...
                    onTap: () {
                      if (party.partyid.isEmpty || party.party.isEmpty) {
                        ...
                        Services()
                            .getPartyWiseItemExportPurchaseFile(
                                context,
                                Helper.toApi(fromdateController.text),
                                Helper.toApi(toDateController.text),
                                party.partyid,
